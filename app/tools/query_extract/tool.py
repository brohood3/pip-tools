...
class QueryExtract:
    def __init__(self):
        ...
        if not self.dune_api_key:
            raise HTTPException(status_code=500, detail="Missing DUNE_API_KEY environment variable")
    ...
    def run(self, prompt: str) -> Dict[str, Any]:
        ...
        if not query_id:
            raise HTTPException(status_code=400, detail="Could not determine Dune query ID. Please provide a valid query ID.")
        ...
        if not answer:
            raise HTTPException(status_code=500, detail="Error extracting information. Please try again.")
    ...
    def _get_dune_results(self, query_id: int) -> Optional[Dict[Any, Any]]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error fetching Dune results: {str(e)}")
    ...
    def _extract_query_details(self, prompt: str) -> tuple[Optional[int], str]:
        ...
        try:
            ...
            if query_id and matched_pattern:
                question = re.sub(matched_pattern, "", question, flags=re.IGNORECASE).strip()
            ...
            raise HTTPException(status_code=400, detail=f"Error parsing query details: {str(e)}") 