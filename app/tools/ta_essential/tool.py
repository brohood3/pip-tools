...
IMPORTANT: Respond with ONLY the raw JSON object. Do not include markdown formatting, code blocks, or any other text. The response should start with {{ and end with }}."""
    ...
    def generate_analysis(self, indicators: Dict[str, Any], symbol: str, interval: str, original_prompt: str) -> str:
            ...
            if ma_type in indicators:
                ...
                for period in ['20', '50', '200']:
                    if f'period_{period}' in indicators[ma_type]:
                        ma_values.append(f"{period} [{indicators[ma_type][f'period_{period}']:.2f}]")
                if ma_values:
                    moving_averages.append(f"• {ma_type.upper()}: {', '.join(ma_values)}")
        ...
        if 'stddev' in indicators and 'value' in indicators['stddev']:
            volume_indicators.append(f"• Standard Deviation: {indicators['stddev']['value']:.2f}")
        
        if volume_indicators:
            indicator_sections.append("Volume & Volatility:\n" + "\n".join(volume_indicators))
        ...
        if momentum_indicators:
            indicator_sections.append("Momentum Indicators:\n" + "\n".join(momentum_indicators))
        ...
        if price_indicators:
            indicator_sections.append("Price Structure:\n" + "\n".join(price_indicators))
        ...
        if 'ichimoku' in indicators:
            ...
            if 'conversion' in ichimoku:
                cloud_indicators.append(f"• Conversion Line (Tenkan-sen): {ichimoku['conversion']:.2f}")
            if 'base' in ichimoku:
                cloud_indicators.append(f"• Base Line (Kijun-sen): {ichimoku['base']:.2f}")
            if 'spanA' in ichimoku:
                cloud_indicators.append(f"• Leading Span A (Senkou Span A): {ichimoku['spanA']:.2f}")
            if 'spanB' in ichimoku:
                cloud_indicators.append(f"• Leading Span B (Senkou Span B): {ichimoku['spanB']:.2f}")
            if 'lagging' in ichimoku:
                cloud_indicators.append(f"• Lagging Span (Chikou Span): {ichimoku['lagging']:.2f}")
            
            if cloud_indicators:
                indicator_sections.append("Ichimoku Cloud:\n" + "\n".join(cloud_indicators))
