...
IMPORTANT: Respond with ONLY the raw JSON object. Do not include markdown formatting, code blocks, or any other text. The response should start with {{ and end with }}."""
                ...
                for indicator_data in batch1_data:
                    if indicator_data.get("result") and not indicator_data.get(
                        ...
                        if indicator_data["indicator"] == "ema":
                            ...
                            result["ema"][f"period_{period}"] = indicator_data[
                                "result"
                            ].get("value")
                        else:
                            valid_indicators += 1
                            result[indicator_data["indicator"]] = indicator_data[
                                "result"
                            ]
    ...
    def generate_analysis(
        ...
        if "ema" in indicators:
            ...
            for period in ["20", "50", "200"]:
                if f"period_{period}" in indicators["ema"]:
                    ema_values.append(
                        f"{period} [{indicators['ema'][f'period_{period}']:.2f}]"
                    )
            if ema_values:
        ...
        if "stoch" in indicators and all(
            k in indicators["stoch"] for k in ["valueK", "valueD"]
        ):
            stoch = indicators["stoch"]
            momentum_indicators.append(
                f"• Stochastic: K[{stoch['valueK']:.2f}], D[{stoch['valueD']:.2f}]"
            )
        ...
        if "willr" in indicators and "value" in indicators["willr"]:
            momentum_indicators.append(
                f"• Williams %R: {indicators['willr']['value']:.2f}"
            )
        ...
        if "stochrsi" in indicators and "value" in indicators["stochrsi"]:
            momentum_indicators.append(
                f"• Stoch RSI: {indicators['stochrsi']['value']:.2f}"
            )
        ...
        for pattern in [
            ...
            if pattern in indicators and "value" in indicators[pattern]:
                pattern_indicators.append(
                    f"• {pattern.title()}: {indicators[pattern]['value']}"
                )
        ...
        if "adosc" in indicators and "value" in indicators["adosc"]:
            volume_indicators.append(
                f"• A/D Oscillator: {indicators['adosc']['value']:.2f}"
            )
        ...
        if "obv" in indicators and "value" in indicators["obv"]:
            volume_indicators.append(
                f"• On Balance Volume: {indicators['obv']['value']:.2f}"
            )

        if "vosc" in indicators and "value" in indicators["vosc"]:
            volume_indicators.append(
                f"• Volume Oscillator: {indicators['vosc']['value']:.2f}"
            )
